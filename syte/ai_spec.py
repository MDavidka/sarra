...
def build_ai_spec(base_url: str = "") -> dict:
    ...
    return {
        ...
        "endpoints": [
            ...
            {"method": "GET", "path": "/api/projects/{uuid}/agent/activity/stream?live=1&since_id=0", "auth": "optional", "description": "SSE real-time agent activity — format=text|jsonl, types= filter"},
        ...
        "agent_session": {
            "description": (
                "Continuous per-workspace Continue cn serve runtime. One hot agent per project; "
            ...
            "sycord_change_flow": [
                ...
                "2. sycord.com opens SSE GET /api/projects/{uuid}/agent/activity/stream?live=1&since_id=N (or internal route with X-Syra-Internal-Secret)",
                "3. sycord.com POST /sycord/api/agent_change {uuid, message, model_profile} — returns request_id immediately",
                "4. Syte emits request_started → token_delta* → file_*/tool_* → message_snapshot → request_completed on SSE",
                "5. Syte matches UUID → workspace, auto-starts cn serve if needed",
                "6. Syte forwards message to Continue CLI POST /message on localhost:{agent_port}",
                "7. Poll GET /state until idle; final reply in request_completed / message_snapshot events",
            ],
            "activity_api": {
            ...
            "workflow": [
                ...
                "2. POST /api/agent_change {uuid, message} — async; save request_id from response",
                "3. GET /api/projects/{uuid}/agent/activity/stream?live=1&since_id=0 — stream token_delta + file events",
                "4. POST /api/agent_settings {uuid, model_profile} — switch profile mid-session",
                "5. GET /api/agent_activity?uuid=&since_id=N — poll snapshot if SSE unavailable",
                "6. POST /api/agent_stop {uuid} — tear down session when done",
            ],
            "status_fields": [
