...
def build_ai_spec(base_url: str = "") -> dict:
    ...
    return {
        ...
        "workflow_continue_agent": [
            "1. Settings → set sycord_ai_bridge_url and sycord_bridge_secret",
            "2. Install Continue CLI on VM: npm install -g @continuedev/cli",
            "3. POST /api/start_agent {uuid, model?} → starts cn serve on port 5000+",
            "4. GET /api/agent_status?uuid= → poll until agent_ready=true",
            "5. Proxy from sycord.com: GET/POST /api/projects/{uuid}/agent/proxy/state|message|…",
            "6. GET /api/agent_logs?uuid= or SSE /api/projects/{uuid}/agent/logs/stream",
            "7. POST /api/stop_agent {uuid} when session ends",
        ],
        "agent_api": {
            "description": "Long-lived Continue cloud agent (cn serve) per workspace — no Continue Hub",
            "integration_contract": "/api/agent_integration.json",
            ...
            "auth": {
                "control": "X-API-Key or Bearer syte token",
                "proxy": "X-API-Key OR X-Sycord-Bridge-Secret (shared secret)",
            },
            "endpoints": {
                "start": "POST /api/start_agent {\"uuid\":\"...\", \"model\": \"syra-base\"}",
                "stop": "POST /api/stop_agent {\"uuid\":\"...\"}",
                "restart": "POST /api/restart_agent {\"uuid\":\"...\", \"model\": \"optional\"}",
                "status": "GET /api/agent_status?uuid=...",
                "logs": "GET /api/agent_logs?uuid=&lines=200",
                "stream": "GET /api/projects/{uuid}/agent/logs/stream?live=1",
                "proxy": "GET|POST /api/projects/{uuid}/agent/proxy/{path}",
            },
            "response_fields": [
                "agent_running", "agent_ready", "agent_port", "agent_status",
                "agent_model", "agent_proxy_url", "agent_state_url", "agent_message_url",
                "bridge_reachable", "bridge_message", "agent_error",
            ],
        },
        "preview_api": {
        ...
        "endpoints": [
            ...
            {"method": "POST", "path": "/api/start_agent", "auth": True, "body": {"uuid": "str", "model": "optional syra-nano|syra-base|syra-havy"}},
            {"method": "POST", "path": "/api/stop_agent", "auth": True, "body": {"uuid": "str"}},
            {"method": "POST", "path": "/api/restart_agent", "auth": True, "body": {"uuid": "str", "model": "optional"}},
            {"method": "GET", "path": "/api/agent_status?uuid=", "auth": True, "description": "agent_ready, agent_proxy_url, bridge_reachable"},
            {"method": "GET", "path": "/api/agent_logs?uuid=&lines=200", "auth": True},
            {"method": "GET", "path": "/api/agent_integration.json", "auth": False, "description": "sycord.com integration contract"},
            {"method": "GET", "path": "/api/projects/{uuid}/agent/logs/stream?live=1", "auth": "optional"},
            {"method": "GET", "path": "/api/projects/{uuid}/agent/proxy/{path}", "auth": "bridge_or_token"},
            {"method": "POST", "path": "/api/projects/{uuid}/agent/proxy/{path}", "auth": "bridge_or_token"},
        ...
        "sycord_api": {
            ...
            "workflow": [
                ...
                "8. POST /sycord/api/agent_start {uuid} — Continue cloud agent",
                "9. GET /sycord/api/agent_status?uuid= — poll agent_ready",
            ],
            ...
            "endpoints": [
                ...
                {"method": "POST", "path": "/sycord/api/agent_start", "auth": True},
                {"method": "GET", "path": "/sycord/api/agent_status?uuid=", "auth": True},
                {"method": "POST", "path": "/sycord/api/agent_stop", "auth": True},
                {"method": "POST", "path": "/sycord/api/agent_restart", "auth": True},
                {"method": "GET", "path": "/sycord/api/agent_integration.json", "auth": False},
