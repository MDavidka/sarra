...
def build_ai_spec(base_url: str = "") -> dict:
    ...
    return {
        ...
        "workflow_create_website_from_git": [
            ...
            "2. POST /api/create_project {name, git_url, branch} → get uuid + stream_url",
        ...
        "workflow_create_website_from_scratch": [
            "1. POST /api/create_project {name, start_command} (no git_url) → uuid",
            "2. POST /api/write_file {uuid, path: 'app/package.json', content: '...'}",
            "3. POST /api/write_file {uuid, path: 'app/Dockerfile', content: '...'}",
            "4. POST /api/execute_command {uuid, command: 'npm install', cwd: 'app'}",
            "5. POST /api/issue_deploy {uuid} → build and start",
        ...
        "endpoints": [
            ...
            {"method": "POST", "path": "/api/create_project", "auth": True, "body": {"name": "str", "uuid": "optional", "git_url": "optional", "git_provider": "optional", "branch": "main", "start_command": "optional", "domain": "optional", "env_vars": {}}},
