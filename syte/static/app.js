...
// Path-style router and complete operator GUI pages.
const SARRA_ROUTE_TO_PAGE = {
  '/':'home','/home':'home','/projects':'projects','/settings':'settings','/profile':'profile','/session':'sessions','/servers':'remote-servers','/users':'users','/audit':'audit-logs','/ssh-keys':'ssh-keys','/ai':'ai','/tags':'tags','/git':'git','/registry':'registry','/secrets':'secrets','/dns':'dns-providers','/s3':'s3-destinations','/certificates':'certificates','/notifications':'notifications','/billing':'billing','/license':'license','/sso':'sso','/docs':'documentation','/support':'support'
};
const SARRA_PAGE_TO_ROUTE = Object.fromEntries(Object.entries(SARRA_ROUTE_TO_PAGE).map(([k,v]) => [v,k]));
Object.assign(PLATFORM_PAGE_LABELS, {home:'Home'});
Object.assign(PLATFORM_PAGE_BLUEPRINTS, {home:{heading:'Ops command center',control:'home-actions',columns:['name','status','created_at']}});
const SARRA_PAGE_FUNCTIONS = {
  home:['Instance health checks API, proxy, Docker daemon','Resource snapshot for CPU, memory, disk','Recent deploys open their project','Failed jobs and agent failures from the last 24h','Preview process count with stop-all-idle control','Certificate expiry warnings inside 30 days','Unreachable remote server list','Quick-create project form','Jump to last opened project','Agent and deploy queue depth','Disk pressure cleanup action','Self-update badge linking to settings'],
  projects:['Search and filter projects','Create project','Rename selected project','Archive and restore','Delete with typed confirmation','Pin and unpin','Clone from Git URL','Preview URL with copy','Last deploy status badge','Open overview','Export UUID','Bulk tag selection'],
  overview:['Start and stop preview','Deploy now','Follow latest deploy log','Open live URL','Bind or unbind domain','Environment variable count and Secrets jump','Health check now','Restart preview process','Rollback last deploy','Git SHA and branch display','Force rebuild','Edit CPU and memory limits'],
  schedules:['List scheduled tasks','Create cron task','Enable or disable','Run one-shot now','Edit cron expression','Delete schedule','Show next five run times','Last run status and duration','View stdout and stderr','Timezone picker','Duplicate job','Pause all project schedules'],
  traefik:['List proxy config files and dynamic routes','Open file in editor','Save and validate YAML or Caddyfile','Diff unsaved edits','Reload Caddy proxy','Syntax check without apply','List live HTTP routes','Toggle route enabled','Add Host rule to upstream port','Add redirect or middleware snippet','Download current config','Restore last good config','Show proxy process status'],
  docker:['List project containers','Start, stop, restart','Tail and follow logs','Inspect image, ports, mounts','Execute one-shot command','Remove stopped containers','Pull image','Compose up or down','Show image size and created time','Prune dangling images after confirm','Copy masked container env','List network membership'],
  settings:['Instance name and FQDN','Default wildcard domain','Proxy engine display and reload','Auto-update toggle and channel','Retention for logs, sessions, artifacts','Default deploy timeout','Preview idle timeout','Backup now and list backups','Restore backup with confirm','Feature flags and rollback switches','Timezone and locale','Download diagnostics bundle'],
  profile:['Edit display name and email','Change password','Avatar or initials','Preferred AI model profile','Notification email toggle','Page-local theme choice','Create API token','Two-factor status','Last login','Delete unused tokens','Export profile JSON','Sign out everywhere'],
  sessions:['Load current operator session','Start bootstrap session','Revoke this session with CSRF','List other sessions','Revoke one or all others','Masked CSRF presence','Same-origin check result','Expiry and idle timeout','Force re-auth','Recent mutations for this session','Agent Turso sessions for last project','Copy session id','Filter active or expired'],
  'remote-servers':['List localhost and remotes','Add host, port, user, key','Validate SSH','Install or detect Docker','Reachability health check','Mark build or runtime','Drain server','Remote CPU, memory, disk','SSH command hint','Remove server','Assign project to server','Show unreachable reason','Re-run host setup'],
  users:['List users','Invite or create','Set role','Disable or enable','Reset password','Force logout','Assign projects','Last seen','Delete user except self','Search','Export CSV','Open audit slice'],
  'audit-logs':['Stream audit events','Filter actor, action, resource, date','Free text search','Open event JSON','Export CSV','Jump to related resource','Severity filter','Live tail','Retention policy link','Mark reviewed','Correlate request id','Settings diff view'],
  'ssh-keys':['List fingerprints','Generate ed25519','Import private or public','Copy public key','Assign key to server','Rotate and keep old until validate','Delete key','Last used','Test key against host','Download .pub','Mark default','Audit creator'],
  ai:['Send agent turn','Poll agent session','List Turso sessions','Resume last','Stop or interrupt','Restart conversation','Thinking level 1-5','Model profile and catalog','MCP edit','Skills add or edit','Failure log','Usage and cost','Subagents','Screenshots and visual analysis','Memory summary'],
  tags:['List tags','Create tag','Rename tag','Change color','Assign to projects','Merge tags','Delete unused','Filter projects by tag','Usage counts','Pin favorites','Export tags','Search tags'],
  git:['List GitHub sources','Connect or rotate token','List repositories','Default branch','Create PR','List PRs','Sync fetch','Webhook status and secret rotation','Disconnect','Last commit for project','Manual deploy from SHA','Test clone'],
  registry:['List registries','Add credentials','Test login','List image tags','Set default pull registry','Mark insecure with warning','Delete credentials','Copy pull snippet','Last pull time','Build-server push target','Rotate password','Show auth method'],
  secrets:['List names only','Create secret','Update write-only','Delete secret','Build vs runtime mark','Assign to project','Copy name','Reveal once with audit','Import .env','Export names template','Search','Unused report'],
  'dns-providers':['List providers','Add masked token','Test credentials','List zones','Create A AAAA CNAME','Resolve check vs server IP','Auto Cloudflare record','Delete record','Default provider','Last sync error','Edit TTL','Disconnect provider'],
  's3-destinations':['List destinations','Add endpoint bucket keys','Test bucket list','Set backup target','Run backup','List remote objects','Restore object to local','Default destination','Delete destination','Usage and last success','Path prefix','Rotate keys'],
  certificates:['List certs and expiry','Issue or request LE','Force renew','SSL resolve repair','Show SANs','Download public cert','Proxy running check','HTTP-01 debug','Attach cert to domain','Delete leftover','Expiry calendar','Copy fingerprint'],
  notifications:['List channels','Add webhook','Test ping','Select events','Enable or disable','Delivery log','Retry last','Quiet hours','Per-project override','Delete channel','Slack or Discord preview','On-call email'],
  billing:['Self-hosted instance statement','Monthly CPU hours disk deploy count','AI token cost','Export usage CSV','Budget cap alert','Per-project costs','Provider quota','Soft-limit agent turns','Invoice-like export','Accounting notes','Reset period','Link to License'],
  license:['Current edition','Paste and validate key','Seat count','Expiry','Feature gates','Remove key','Offline fingerprint','Copy support bundle id','Refresh validation','Key history','Compliance text','Open source notice'],
  sso:['List OIDC and GitHub providers','Add client and issuer','Test discovery','Enable or disable','Require SSO','Allowed email domain','Map group to role','Login preview URL','Disable password login','Last SSO login','Remove provider','JIT create toggle'],
  documentation:['Render README and docs','Search docs','Open API catalog','Copy endpoint snippet','Agent streaming contract','Turso persistence explainer','Version and commit','GitHub repo link','Print or export doc','Last updated','Keyboard cheatsheet','Remote server runbook'],
  support:['Generate diagnostics zip','Copy version and stats','Open GitHub issues','Prefilled new issue','Proxy Docker Agent status','Last 20 errors','Operator email','Restart service if allowed','Clear preview cache','Support notes','Attach audit slice','Mark incident ongoing']
};
const SARRA_VISUAL = {home:'home',projects:'projects',overview:'overview',schedules:'schedules',traefik:'proxy',docker:'docker',settings:'settings',profile:'profile',sessions:'session','remote-servers':'servers',users:'users','audit-logs':'audit','ssh-keys':'ssh-keys',ai:'ai',tags:'tags',git:'git',registry:'registry',secrets:'secrets','dns-providers':'dns','s3-destinations':'s3',certificates:'certificates',notifications:'notifications',billing:'billing',license:'license',sso:'sso',documentation:'docs',support:'support'};
renderDedicatedPage = function(page, data) {
  const target = document.getElementById('platform-dedicated-page'); if (!target) return;
  const list = data.resources || []; const visual = SARRA_VISUAL[page] || page;
  const funcs = SARRA_PAGE_FUNCTIONS[page] || [];
  const rows = list.slice(0, 8).map((row) => `<tr><td>${esc(String(row.name||row.key||row.event||row.uuid||'Resource'))}</td><td>${esc(String(row.status||row.state||row._table||'tracked'))}</td><td><button class="btn-pill btn-ghost" data-copy-text="${esc(String(row.uuid||row.id||''))}">Copy</button></td></tr>`).join('') || '<tr><td colspan="3">No records yet; controls are live and persist through Syte APIs.</td></tr>';
  const sessionPanel = page === 'sessions' ? '<div id="sarra-session-panel" class="sarra-timeline"><p>Loading current operator session…</p></div>' : '';
  target.innerHTML = `<section class="sarra-page page-${visual}"><div class="sarra-hero"><div><p class="dedicated-eyebrow">${esc(PLATFORM_PAGE_LABELS[page]||page)}</p><h2>${esc(data.title||PLATFORM_PAGE_LABELS[page]||page)}</h2><p>${esc(data.description||'Deep-linkable operator workspace backed by Syte APIs.')}</p></div><button class="btn-pill btn-primary" data-sarra-action="refresh"><i data-lucide="refresh-cw"></i>Refresh</button></div><div class="sarra-kpis"><div><span>Records</span><strong>${esc(String(data.resource_count||0))}</strong></div><div><span>API</span><strong>Live</strong></div><div><span>Route</span><strong>${esc(SARRA_PAGE_TO_ROUTE[page]||location.pathname)}</strong></div></div>${sessionPanel}<div class="sarra-layout sarra-${visual}"><form class="sarra-control-card"><h3>Primary controls</h3>${funcs.slice(0,15).map((f,i)=>`<button type="button" class="ops-card" data-sarra-op="${i}"><span>${i+1}</span>${esc(f)}</button>`).join('')}</form><div class="sarra-inspector"><h3>Live data</h3><table><tbody>${rows}</tbody></table><textarea class="sarra-editor" spellcheck="false"># ${esc(PLATFORM_PAGE_LABELS[page]||page)} workspace\n# APIs: /api/platform/navigation/${esc(page)}, /api/operator/session, project/runtime APIs where available.</textarea></div></div></section>`;
  target.querySelectorAll('[data-sarra-op]').forEach(btn=>btn.addEventListener('click', async()=>{ try{ const msg=document.getElementById('platform-page-message'); if(msg) msg.textContent='Working…'; await api(`/platform/navigation/${encodeURIComponent(page)}/actions`,{method:'POST',body:JSON.stringify({action:funcs[Number(btn.dataset.sarraOp)]||'operator-action'})}); if(msg) msg.textContent='Action completed and audited.'; await loadPlatformPage(page);}catch(e){const msg=document.getElementById('platform-page-message'); if(msg) msg.textContent=`Action failed: ${e.message}`;}}));
  target.querySelectorAll('[data-copy-text]').forEach(btn=>btn.addEventListener('click',()=>navigator.clipboard?.writeText(btn.dataset.copyText||'')));
  target.querySelector('[data-sarra-action="refresh"]')?.addEventListener('click',()=>loadPlatformPage(page));
  if (page === 'sessions') loadSarraSessionPanel();
  refreshIcons();
};
async function loadSarraSessionPanel(){ const el=document.getElementById('sarra-session-panel'); if(!el) return; try{ const s=await api('/operator/session'); el.innerHTML=`<div><strong>${s.authenticated?'Authenticated':'Anonymous'}</strong><small>session ${esc((s.session_id||'none').slice(0,12))}</small></div><div><span>CSRF</span><code>${s.csrf_token?'••••'+s.csrf_token.slice(-6):'missing'}</code></div><div><span>Same-origin</span><code>protected on mutations</code></div><button class="btn-pill btn-ghost" id="copy-current-session">Copy session id</button><button class="btn-pill btn-ghost" id="revoke-current-session">Revoke this session</button>`; document.getElementById('copy-current-session')?.addEventListener('click',()=>navigator.clipboard?.writeText(s.session_id||'')); document.getElementById('revoke-current-session')?.addEventListener('click',async()=>{await api('/operator/session',{method:'DELETE'}); await loadSarraSessionPanel();}); }catch(e){ el.innerHTML=`<div class="platform-error">Session API error: ${esc(e.message)}</div><button class="btn-pill btn-primary" id="start-session">Start session</button>`; document.getElementById('start-session')?.addEventListener('click',()=>showLoginScreen?.('platform')); }}
function sarraNavigate(path, replace=false){ const projectMatch=path.match(/^\/projects\/([^/]+)\/(overview|schedules|proxy|docker)$/); if(projectMatch){ activeServiceId=projectMatch[1]; const page={proxy:'traefik'}[projectMatch[2]]||projectMatch[2]; activePlatformPage=page; showView('platform'); } else { const page=SARRA_ROUTE_TO_PAGE[path]||'home'; activePlatformPage=page; showView('platform'); } if(!replace && location.pathname!==path) history.pushState({path},'',path); }
document.querySelectorAll('.nav-sublink[data-view]').forEach((el)=>{ el.addEventListener('click',(event)=>{ const platformPage=el.dataset.platformPage; let path=null; if(platformPage){ event.preventDefault(); path=SARRA_PAGE_TO_ROUTE[platformPage]||'/home'; } else if(el.dataset.view==='dashboard'){ path='/home'; } else if(el.dataset.view==='users'){ path='/users'; } else if(el.dataset.view==='settings'){ path='/settings'; } if(path){ event.preventDefault(); sarraNavigate(path); } }); });
window.addEventListener('popstate',()=>sarraNavigate(location.pathname,true));
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded',()=>sarraNavigate(location.pathname,true)); else setTimeout(()=>sarraNavigate(location.pathname,true),0);
