...
function renderAiDebug(report) {
  ...
  el.innerHTML = `
    ...
    <div>
      ...
      <div class="hint">status ${esc(agent.agent_status || '—')} · port ${agent.agent_port ?? '—'} · CLI ${(report.opencode_cli || report.continue_cli)?.installed ? esc((report.opencode_cli || report.continue_cli).version || 'installed') : 'missing'}</div>
    ...
    ${config.snippet ? `<div><strong>opencode.json</strong><pre class="ai-debug-config">${esc(config.snippet)}</pre></div>` : ''}
